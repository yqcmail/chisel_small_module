function! Chi_sta_cnt_seq(...)
if(len(a:000)==0) ||(a:000[0]== "h" || a:000[0]== "-h" || a:000[0]== "help" )
   echo "Chi_sta_cnt_seq(sta_name0,sta_name1,....)"
else
...
"""" 
  ...
  call add(str_ary, " val cnt_s = RegInit(0.U(log2Up(Dw+2).W)")
  call add(str_ary, "")
  ...
  call add(str_ary, "   is("."s".idx."_".ary_sta[idx]."){")
    if(idx == 0 )
     call add(str_ary,"     cnt_s := 0.U ")
     call add(str_ary,"     when(start_i === 1.U){")
     call add(str_ary,"       rg_sta := "."s".(idx+1)."_".ary_sta[idx+1])
     call add(str_ary,"      }")
    else
     call add(str_ary,"     cnt_s := cnt_s + 1.U ")
     call add(str_ary,"    when( cnt_s >= (Dw-1).asUInt){")
